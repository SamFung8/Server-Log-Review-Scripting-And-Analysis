...
mailStatus = "Log Review(SSLVPN) - "
mailBody = ""
mailBody2 = ""

#fileName = (todate - timedelta(days = 7)).strftime("%Y%m%d") + " To " + (todate - timedelta(days = 1)).strftime("%Y%m%d")
fileName = (todate - timedelta(days = 7)).strftime("%Y%m%d") + " To " + (todate).strftime("%Y%m%d")

message = EmailMessage()
message['From'] = 'team5@housingauthority.gov.hk'
#message['To'] = 'kingshun.fung@housingauthority.gov.hk'
message['To'] = 'kingshun.fung@housingauthority.gov.hk, tonywp.ho@housingauthority.gov.hk, pingfuk.lam@housingauthority.gov.hk, pk.cheung@housingauthority.gov.hk, andywk.lau@housingauthority.gov.hk'

path = r"E:/"
#path = 'C:/Users/KingShunFung/Desktop/log_data'

keywords = ["login", "logout", "major", "error"]
keywords_2 = ["bytes read and 0 bytes written"]
files = []
dateList = []
#ipList = ["172.29.1.57", "172.29.1.58"]
ipList = ["172.31.1.116", "172.31.1.117"]

for i in range(61):
    strKey = "after " + str(i) + " seconds, with"
    keywords_2.append(strKey)
...
for f in files:
    ...
    for line in fileinput.input(files=logFile, encoding='unicode_escape'):
        ...
        for keyword in keywords_2:
            if keyword in lower_line:
                mailBody2 = mailBody2 + line
                break

attachment = mailBody.encode('utf-8')
attachment2 = mailBody2.encode('utf-8')
...
message.add_attachment(attachment2, maintype='application', subtype='log', filename=fileName + ' traffic.log')
